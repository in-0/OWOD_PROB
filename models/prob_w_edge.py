...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
                 aux_loss=True, with_box_refine=False, two_stage=False, unmatched_boxes=False, featdim=1024):
    ...
    def forward(self, samples: NestedTensor):
        ...
        if self.num_feature_levels > len(srcs):
            _len_srcs = len(srcs)
            for layer in range(_len_srcs, self.num_feature_levels):
                if layer == _len_srcs:
                    src = self.input_proj[layer](features[-1].tensors)
                else:
                    src = self.input_proj[layer](srcs[-1])
                m = samples.mask
        ...
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1], 'pred_obj':outputs_objectness[-1], 'resnet_1024_feat':resnet_1024_feature} 
...
class SetCriterion(nn.Module):
    ...
    """
    def __init__(self, args, num_classes, matcher, weight_dict, losses, invalid_cls_logits,
            hidden_dim, focal_alpha=0.25, empty_weight=0.1):
    ...
    def loss_obj_likelihood(self, outputs, targets, indices, num_boxes, owod_targets, owod_indices):
    ...
    def forward(self, samples, outputs, targets, epoch):
        ...
        for loss in self.losses:
            kwargs = {}
            losses.update(self.get_loss(loss, outputs, targets, indices, num_boxes, owod_targets, owod_indices, **kwargs))
            ...
            for i, aux_outputs in enumerate(outputs['aux_outputs']):
                ...
                for loss in self.losses:
                    ...
                    if loss == 'labels':
                        ...
                        kwargs['log'] = False
                    l_dict = self.get_loss(loss, aux_outputs, targets, indices, num_boxes, owod_targets, owod_indices, **kwargs)
